...
func main() {
	...
	matchingInstruments := inventory.searchGuitar(whatErinLikes)
	...
	for _, instrument := range matchingInstruments {
		spec := instrument.getSpec().(*GuitarSpec)
		fmt.Printf("  We have a %v %v %v guitar:\n     %v back and sides,\n     %v top.\n     %v strings.\n  You can have it for only $%.2f!\n  ----\n",
			spec.instrumentSpec.getBuilder(),
			spec.instrumentSpec.getModel(),
			spec.instrumentSpec.getInstrumentType(),
			spec.instrumentSpec.getBackWood(),
			spec.instrumentSpec.getTopWood(),
			spec.getNumStrings(),
			instrument.getPrice())
	}
}
...
func initializeInventory() (*Inventory, error) {
	...
	type guitarData struct {
		serialNumber string
		price        float64
		spec         *GuitarSpec
	}

	guitarsToAdd := []guitarData{
	...
	for _, g := range guitarsToAdd {
		if err := inventory.addInstrument(g.serialNumber, g.price, g.spec); err != nil {
			fmt.Println("warning: could not add guitar", g.serialNumber, ":", err)
		}
	}
